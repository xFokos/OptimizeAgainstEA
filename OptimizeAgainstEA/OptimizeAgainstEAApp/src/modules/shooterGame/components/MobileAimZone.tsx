...
export function MobileAimZone({ inputRef }: Props) {
    ...
    useEffect(() => {
        ...
        const updateAim = (cx: number, cy: number) => {
            ...
            if (len > DEADZONE) {
                ...
                // Ziel weit in Joystick-Richtung → Winkel stimmt unabhängig von Spielerposition
                inputRef.current.mouseX = ARENA.WIDTH  / 2 + Math.cos(angle) * 5000;
                inputRef.current.mouseY = ARENA.HEIGHT / 2 + Math.sin(angle) * 5000;
            }
        };
